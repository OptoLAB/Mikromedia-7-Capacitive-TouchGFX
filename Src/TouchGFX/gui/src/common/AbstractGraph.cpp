#include <gui/common/AbstractGraph.hpp>
...
void AbstractGraph::setRange(int newLeft, int newRight, int newBottom, int newTop)
{
    if (left != newLeft || right != newRight || top != newTop || bottom != newBottom)
    {
        assignRange(newLeft, newRight, newTop, newBottom);
